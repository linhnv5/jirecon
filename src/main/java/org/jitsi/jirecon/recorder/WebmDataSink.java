	...
	implements DataSink, BufferTransferHandler
{
	...
	/**
	 * Whether we are in a state of waiting for a keyframe and discarding non-key
	 * frames.
	 */
	...
	/**
	 * Property name to control auto requesting keyframes periodically to improve
	 * seeking speed without re-encoding the file
	 */
		...
		synchronized (openCloseSyncRoot) {
			...
			if (waitingForKeyframe && key) {
				if (valid) {
					...
					if (logger.isInfoEnabled())
						logger.info("Received the first keyframe (width=" + width + "; height=" + height + ")" + " ssrc=" + ssrc);
					...
					if (logger.isInfoEnabled())
						logger.info("Received an invalid first keyframe. " + "Requesting a new one." + ssrc);
				}
			}
			...
			if (!waitingForKeyframe) {
				if (key) {
					...
					if (width != oldWidth || height != oldHeight) {
						if (logger.isInfoEnabled()) {
							logger.info("VP8 stream width/height changed. Old: " + oldWidth + "/" + oldHeight
									+ ". New: " + width + "/" + height + ".");
						}
					}
				}
	...
	/**
	 * Returns <tt>true</tt> if the VP8 compressed frame contained in <tt>buf</tt>
	 * at offset <tt>offset</tt> is a keyframe. TODO: move it to a more general
	 * class?
	 *
	 ...
	 *               starts.
	 *
	 * @return <tt>true</tt>if the VP8 compressed frame contained in <tt>buf</tt> at
	 *         offset <tt>offset</tt> is a keyframe.
	 */
	 ...
	 * <tt>buf</tt> at offset <tt>offset</tt> is valid. TODO: move it to a more
	 * general class?
	 *
	 * @param buf    the buffer containing a compressed VP8 frame.
	 * @param offset the offset in <tt>buf</tt> where the VP8 compressed frame
	 *               starts.
	 *
	 * @return <tt>true</tt>if the VP8 compressed keyframe contained in <tt>buf</tt>
	 *         at offset <tt>offset</tt> is valid.
	 */
	private boolean isKeyFrameValid(byte[] buf, int offset) {
		return (buf[offset + 3] == (byte) 0x9d) && (buf[offset + 4] == (byte) 0x01) && (buf[offset + 5] == (byte) 0x2a);
	}

	/**
	 * Returns the width of the VP8 compressed frame contained in <tt>buf</tt> at
	 * offset <tt>offset</tt>. See the format defined in RFC6386. TODO: move it to a
	 * more general class?
	 *
	 * @param buf    the buffer containing a compressed VP8 frame.
	 * @param offset the offset in <tt>buf</tt> where the VP8 compressed frame
	 *               starts.
	 *
	 * @return the width of the VP8 compressed frame contained in <tt>buf</tt> at
	 *         offset <tt>offset</tt>.
	 */
	...
	/**
	 * Returns the height of the VP8 compressed frame contained in <tt>buf</tt> at
	 * offset <tt>offset</tt>. See the format defined in RFC6386. TODO: move it to a
	 * more general class?
	 *
	 * @param buf    the buffer containing a compressed VP8 frame.
	 ...
	 *               starts.
	 *
	 * @return the height of the VP8 compressed frame contained in <tt>buf</tt> at
	 *         offset <tt>offset</tt>.
	 */
	...
	/**
	 * Returns the value of the <tt>show_frame</tt> field from the "uncompressed
	 * data chunk" in the VP8 compressed frame contained in <tt>buf</tt> at offset
	 * <tt>offset</tt>. RFC6386 isn't clear about the format, so the interpretation
	 * of
	 * 
	 * @{link http://tools.ietf.org/html/draft-ietf-payload-vp8-11} is used. TODO:
	 *        move it to a more general class?
	 *
	 * @param buf    the buffer containing a compressed VP8 frame.
	 * @param offset the offset in <tt>buf</tt> where the VP8 compressed frame
	 *               starts.
	 *
	 * @return the value of the <tt>show_frame</tt> field from the "uncompressed
	 *         data chunk" in the VP8 compressed frame contained in <tt>buf</tt> at
	 *         offset <tt>offset</tt>.
	 */
