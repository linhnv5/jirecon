...
//    private static final Format ulpfecFormat = new VideoFormat(Constants.ULPFEC);
	 ...
	 */
	private static final String FMJ_VIDEO_JITTER_BUFFER_MIN_SIZE_PNAME = RecorderRtpImpl.class.getCanonicalName()
	 ...
	 */
	private static final String FMJ_AUDIO_JITTER_BUFFER_MIN_SIZE_PNAME = RecorderRtpImpl.class.getCanonicalName()
	...
	/**
	 * The name of the property which controls whether the recorder should perform
	 * active speaker detection.
	 */
	private static String PERFORM_ASD_PNAME = RecorderRtpImpl.class.getCanonicalName() + ".PERFORM_ASD";

	/**
	 * The name of the property which sets a custom output audio codec. Currently
	...
	/**
	 * The instance which should be notified when events related to recordings (such
	 * as the start or end of a recording) occur.
	 */
	private RecorderEventHandlerImpl eventHandler;

	/**
	 * Holds the <tt>ReceiveStreams</tt> added to this instance by
	 * {@link #rtpManager} and additional information associated with each one (e.g.
	 * the <tt>Processor</tt>, if any, used for it).
	 */
	 ...
	 * streams of this <tt>RecorderRtpImpl</tt> and notify it about changes to the
	 * active speaker via calls to {@link #activeSpeakerChanged(long)}
	 */
	private ActiveSpeakerDetector activeSpeakerDetector = null;

	/**
	 * Controls whether this <tt>RecorderRtpImpl</tt> should perform active speaker
	 * detection and fire <tt>SPEAKER_CHANGED</tt> recorder events.
	 */
...
//				AUDIO_CONTENT_DESCRIPTOR = new ContentDescriptor(FileTypeDescriptor.WAVE);
	...
	/**
	 * Implements {@link Recorder#getFilename()}. Returns null, since we don't have
	 * a (single) associated filename.
	 */
	@Override
	...
	/**
	 * Sets the instance which should be notified when events related to recordings
	...
	public void start(String format, String dirname) throws IOException, MediaException {
		...
		/*
		 * Register a fake call participant. TODO: can we use a more generic MediaStream
		 * here?
		 */
		mediaStream = mediaService.createMediaStream(new MediaDeviceImpl(new CaptureDeviceInfo(), MediaType.VIDEO));

		/*
		 * Note that we use only one RTPConnector for both the RTPTranslator and the
		 * RTPManager instances. The this.translator will write to its output streams,
		 * and this.rtpManager will read from its input streams.
		 */
		...
		/*
		 * Note: When this.rtpManager sends RTCP sender/receiver reports, they will end
		 * up being written to its own input stream. This is not expected to cause
		 * problems, but might be something to keep an eye on.
		 */
	...
	public void update(ReceiveStreamEvent event) {
		...
		if (event instanceof NewReceiveStreamEvent) {
			...
			/*
			 * XXX TODO IRBABOON This is a terrible hack which works around a failure to
			 * realize() some of the Processor-s for audio streams, when multiple streams
			 * start nearly simultaneously. The cause of the problem is currently unknown
			 * (and synchronizing all FMJ calls in RecorderRtpImpl does not help). XXX TODO
			 * NOOBABRI
			 */
			if (receiveStreamDesc.format instanceof AudioFormat) {
				...
				new Thread() {
					...
					public void run() {
						...
						try {
							...
							logger.warn("Sleeping for " + ms + "ms before" + " configuring processor for SSRC=" + ssrc
			...
			if (receiveStreamDesc != null) {
				...
				if (logger.isInfoEnabled()) {
					logger.info("ReceiveStream timeout for an unknown stream" + " (already removed?) "
	...
	private void removeReceiveStream(ReceiveStreamDesc receiveStream, boolean emptyJB) {
		...
		if (receiveStream.format instanceof VideoFormat) {
			 ...
			 * direction changes: if the stream with the same SSRC starts again later, we
			 * will obtain new mappings based on the new Sender Reports. See
	...
	public void controllerUpdate(ControllerEvent ev) {
		...
		if (ev instanceof ConfigureCompleteEvent) {
			if (logger.isInfoEnabled()) {
				logger.info("Configured processor for ReceiveStream ssrc=" + desc.ssrc + " (" + desc.format + ")" + " "
						+ System.currentTimeMillis());
			}
			...
			if (audio) {
				...
				if (!AUDIO_CONTENT_DESCRIPTOR.equals(cd)) {
					logger.error("Failed to set the Processor content " + "descriptor to " + AUDIO_CONTENT_DESCRIPTOR
							+ ". Actual result: " + cd);
	 ...
	 * 
	 * @param ssrc the SSRC for which to restart recording. RTP packet of the new
	 *             recording).
	 */
	private void resetRecording(long ssrc, long timestamp) {
		...
		if (receiveStream != null && receiveStream.format instanceof AudioFormat) {
			String newFilename = getNextFilename(path, ssrc, true, AUDIO_FILENAME_SUFFIX);
	...
	/**
	 * Handles a request from a specific <tt>DataSink</tt> to request a keyframe by
	 * sending an RTCP feedback FIR message to the media source.
	 *
	...
	private boolean requestFIR(WebmDataSink dataSink) {
		...
		if (desc != null && rtcpFeedbackSender != null)
			return rtcpFeedbackSender.requestKeyframe(desc.ssrc);
		return false;
	}
	 ...
	 * hanging, and on failure to find an inexistant filename the method will return
	 * null.
	 *
	 * @param prefix
	 * @param suffix
	 * @return
	 */
	private String getNextFilename(String prefix, long ssrc, boolean audio, String suffix) {
		...
		try {
			...
			prefix = prefix + File.separator + ssrc;
		}
	 ...
	 * @return the <tt>ReceiveStreamDesc</tt> with a particular <tt>Processor</tt>,
	 *         or <tt>null</tt>.
	 */
	 ...
	 * @return the <tt>ReceiveStreamDesc</tt> with a particular <tt>DataSink</tt>,
	 *         or <tt>null</tt>.
	 */
	 ...
	 * converting the negative <tt>int</tt> values sometimes resulting from reading
	 * of a 32-bit field into the correct unsigned <tt>long</tt> value. So do the
	 * conversion here.
	 *
	 ...
	 * Notifies this <tt>RecorderRtpImpl</tt> that the audio <tt>ReceiveStream</tt>
			...
			if (!activeVideoSsrcs.contains(ssrc)) {
				synchronized (activeVideoSsrcs) {
					if (!activeVideoSsrcs.contains(ssrc)) {
						...
						rtcpFeedbackSender.requestKeyframe(ssrc);
					}
				}
			}
		}
	...
	private void emptyPacketBuffer(long ssrc) {
		...
		for (RawPacket pkt : pkts)
			dataStream.write(pkt.getBuffer(), pkt.getOffset(), pkt.getLength(), false /* already transformed */);
	}
	...
	private class RTPConnectorImpl implements RTPConnector {
		...
		private RTPConnectorImpl(byte redPT, byte ulpfecPT) {
			...
			// incoming packets.
			TransformEngine transformEngine = new TransformEngineChain(
					new TransformEngine[] { packetBuffer, new TransformEngineImpl(), new CompoundPacketEngine(),
							new FECTransformEngine(FECTransformEngine.FecType.ULPFEC, ulpfecPT, (byte) -1, mediaStream),
		...
		private class OutputDataStreamImpl implements OutputDataStream {
			...
			public synchronized int write(byte[] buffer, int offset, int length, boolean transform) {
				...
				if (transform) {
					PacketTransformer packetTransformer = isControlStream ? rtcpPacketTransformer
		...
		private class PushSourceStreamImpl implements PushSourceStream {
			 ...
			 * {@inheritDoc}
			 *
			 * We keep the first non-null <tt>SourceTransferHandler</tt> that was set,
			 * because we don't want it to be overwritten when we initialize a second
			 * <tt>RTPManager</tt> with this <tt>RTPConnector</tt>.
			 *
		...
		/**
		 * A transform engine implementation which allows <tt>RecorderRtpImpl</tt> to
		 * intercept RTP and RTCP packets in.
		 */
	...
	private class ReceiveStreamDesc {
		...
		/**
		 * The <tt>Processor</tt> used to transcode this receive stream into a format
		 * appropriate for saving to a file.
		 */
		 ...
		 * <tt>DataSink</tt> (i.e. the <tt>DataSource</tt> "after" all needed
		 * transcoding is done).
		 */
