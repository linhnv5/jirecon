...
public class WebRtcDataStreamManager
{
     ...
     * @param dtlsControl
     */
    public void runAsServer(StreamConnector connector, MediaStreamTarget streamTarget, DtlsControl dtlsControl)
    {
        try
        {
     ...
     * @param dtlsControl
     */
    public void runAsClient(StreamConnector connector, MediaStreamTarget streamTarget, DtlsControl dtlsControl)
    {
        try
        {
            initSctp(connector, streamTarget, dtlsControl);
	...
	private void initSctp(StreamConnector connector, MediaStreamTarget streamTarget, DtlsControl dtlsControl)
        throws Exception
    {
        if (null != sctpSocket)
        {
        ...
        final DtlsTransformEngine engine = (DtlsTransformEngine) dtlsControl.getTransformEngine();
        ...
        sctpSocket.setLink(new IceUdpDtlsLink(sctpSocket, iceUdpSocket, transformer));
        ...
        long reliab, int sid, String label) throws IOException
    {
        ...
        int sentCount = sctpSocket.send(packet.array(), true, sid, WebRtcDataStream.WEB_RTC_PPID_CTRL);
