...
public class IceUdpTransportManager
{
    ...
    public IceUdpTransportManager()
    {
        ...
        MIN_STREAM_PORT = configuration.getInt(ConfigurationKey.MIN_STREAM_PORT_KEY, -1);
        MAX_STREAM_PORT = configuration.getInt(ConfigurationKey.MAX_STREAM_PORT_KEY, -1);
    }
    ...
    public boolean wrapupConnectivityEstablishment()
    {
        ...
        PropertyChangeListener propertyChangeListener = new PropertyChangeListener()
        {
            ...
            public void propertyChange(PropertyChangeEvent ev)
            {
                ...
                {
                    Agent iceAgent2 = (Agent) ev.getSource();

                    iceAgent2.removeStateChangeListener(this);
                    if (iceAgent2 == IceUdpTransportManager.this.iceAgent)
                    {
        ...
        throws Exception
    {
        ...
        try
        {
            iceAgent.createComponent(stream, Transport.UDP, getPreferredPort(), MIN_STREAM_PORT, MAX_STREAM_PORT);
        ...
        {
            throw new Exception("Could not create ICE component, " + e.getMessage());
        }
    }
    ...
    public void addRemoteCandidates(Map<MediaType, IceUdpTransportPacketExtension> transportPEs)
    {
        ...
        for (Map.Entry<MediaType, IceUdpTransportPacketExtension> e : transportPEs.entrySet())
        {
            ...
            for (CandidatePacketExtension candidate : candidates)
            {
                ...
                final Component component = stream.getComponent(candidate.getComponent());
                ...
                final RemoteCandidate relatedCandidate = component.findRemoteCandidate(relatedAddress);

                final TransportAddress mainAddress = new TransportAddress(candidate.getIP(), candidate.getPort(), Transport.parse(candidate.getProtocol()));

                final RemoteCandidate remoteCandidate = new RemoteCandidate(mainAddress, component,
    ...
    private List<CandidatePacketExtension> createLocalCandidatePacketExts(MediaType mediaType)
    {
        ...
        {
            CandidatePacketExtension packetExt = new CandidatePacketExtension();
            packetExt.setComponent(candidate.getParentComponent().getComponentID());
            ...
            packetExt.setType(org.jitsi.xmpp.extensions.jingle.CandidateType.valueOf(candidate.getType().toString()));
        ...
        if (stream != null)
        {
            ...
            for (Component component : stream.getComponents())
            {
                if (component != null)
                {
                    ...
                    {
                        InetSocketAddress address = selectedPair.getRemoteCandidate().getTransportAddress();
            ...
            streamTarget = new MediaStreamTarget(rtpAddress, rtcpAddress);
        ...
        throws Exception
    {
        ...
        IceMediaStream stream = getIceMediaStream(mediaType);
        if (stream == null)
            throw new Exception("Could not get stream connector, ICE media stream was not prepared.");
        ...
        streamConnector = new DefaultStreamConnector(rtpSocket, rtcpSocket, mediaType == MediaType.DATA);
