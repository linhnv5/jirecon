    ...
    implements TaskManagerEvent.JireconEventListener
{
     ...
     * Logger.
     */
    public static Logger logger = Logger.getLogger(XMPPComponent.class.getName());
    ...
    public void handleEvent(TaskManagerEvent evt)
    {
        ...
        synchronized (recordingSessions)
        {
            ...
            if (TaskManagerEvent.Type.TASK_ABORTED == evt.getType())
            {
                ...
                notification = createIqSet(session,
    ...
    {
        String mucJid = RecordingIqUtils.getAttribute(iq, RecordingIqUtils.MUCJID_NAME);
        ...
        synchronized (recordingSessions)
        {
            for (RecordingSession session : recordingSessions)
            {
                ...
                {
                    logger.log(Level.SEVERE, "Failed to start a recording session, already recording.");
    ...
    {
        final String rid = RecordingIqUtils.getAttribute(iq, RecordingIqUtils.RID_NAME);
     ...
     * @return Set IQ.
     */
    private IQ createIqSet(RecordingSession session, String status, String rid, String dst)
    {
        IQ set = RecordingIqUtils.createIqSet(localJid, session.getClientJid().toString());
